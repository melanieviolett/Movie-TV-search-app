...
const App = () => {
  ...
  const movieTvRequest = async (searchVal) => {
    ...
    const url = `http://www.omdbapi.com/?i=tt3896198&s=${searchVal}&apikey=${process.env.REACT_APP_API_KEY}`;
  ...
  useEffect(() => {
    if (localStorage.getItem("react-movie-tv-app-favs")) {
  ...
  const rmFavMovie = (movie) => {
    const newFavList = favs.filter((fav) => fav.imdbID !== movie.imdbID);
    setFavs(newFavList);
    saveToLocalStorage(newFavList);
  };
    ...
    <>
      <div className="w-full h-auto relative">
        ...
        <div className="flex flex-row w-full h-auto relative">
          <Movie_TvList
            movies={favs}
            handleFavsClick={rmFavMovie}
            favoriteComponent={RmFavorites}
          ></Movie_TvList>
        </div>
      </div>
    </>
